...
bool insertIfNotHave(brd b) {
  brd hashValue = b;
  while (true) {
    if (hash[hashValue % HASH_SIZE] == 0) {
      hash[hashValue % HASH_SIZE] = b;
      return true;
    } else if (hash[hashValue % HASH_SIZE] == b) {
      return false;
    }
    hashValue = (brd)((((unsigned long long)hashValue * (unsigned long long)b) >> 16) & 0xFFFFFFFF);
  }
}
  ...
  while(!before->empty()) {
    for (int i = 0; i < before->size(); i++) {
      ...
      for (int j = 0; j < rotateLength[emptySrcIndex]; j++) {
        ...
        if (step < 30 && insertIfNotHave(next)) {
